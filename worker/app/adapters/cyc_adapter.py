"""Scraping adapter for CyC Computer (cyccomputer.pe)."""
...
ALLOWED_CYC_HOSTS = {"cyccomputer.pe", "www.cyccomputer.pe"}


class CycComputerAdapter(BaseStoreAdapter):
    """Scraping adapter for CyC Computer (PrestaShop e-commerce)."""

    def __init__(self, http_client: SafeHttpClient | None = None) -> None:
        self.client = http_client or SafeHttpClient(allowed_hosts=ALLOWED_CYC_HOSTS)

    def fetch_product_price(self, store_product_url: str) -> ScrapedPriceResult:
        """Fetch and extract structured product data from CyC Computer product URL."""
    ...
    def parse_html(self, html_content: str, source_url: str) -> ScrapedPriceResult:
        ...
        if not name and not sku and price is None:
            raise FatalScrapingError(
                f"Missing required product fields in CyC Computer page: {source_url}"
            )
    ...
    def _extract_availability(self, soup: BeautifulSoup) -> Availability:
        ...
        avail_el = soup.find(id=re.compile(r"product-availability", re.I)) or soup.find(
            class_=re.compile(r"product-availability|availability|product-quantities", re.I)
        )
        if avail_el:
        ...
        # 3. Check for ambiguous 'consultar disponibilidad' or consult
        if "consultar disponibilidad" in combined_text or "consultar" in combined_text:
            return "unknown"

        # Default to unknown if no clear stock indicator is found
        return "unknown"
    ...
    def _extract_price(
        ...
        CyC Computer publishes a base_price for cash or bank transfer (*Sin recargo por
        ...
        if not price_el:
            if availability in ("out_of_stock", "unknown"):
                return None, None
            raise FatalScrapingError("Missing price for in_stock product on CyC Computer")
