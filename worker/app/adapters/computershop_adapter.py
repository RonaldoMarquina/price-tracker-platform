...
class ComputerShopAdapter(BaseStoreAdapter):
    ...
    def _extract_image_url(self, soup: BeautifulSoup) -> str | None:
        ...
        if og_img and og_img.get("content"):
            validated = validate_store_image_url(og_img.get("content"), ALLOWED_COMPUTERSHOP_HOSTS)
            if validated:
                return validated
        ...
        if img_el:
            ...
            validated = validate_store_image_url(candidate, ALLOWED_COMPUTERSHOP_HOSTS)
            if validated:
                return validated
