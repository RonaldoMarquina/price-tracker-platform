.PHONY: help up down test-backend test-worker test-frontend test-all lint-backend lint-worker lint-frontend lint-all format migrate migrate-down seed
...
help:
	...
	@echo "  make migrate        - Aplica las migraciones de Alembic (upgrade head)"
	@echo "  make migrate-down   - Revierte una migración de Alembic"
	@echo "  make seed           - Inserta datos mínimos de desarrollo (idempotente)"
